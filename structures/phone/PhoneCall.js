...
module.exports = class PhoneCall {
	...
	async start() {
		if (this.adminCall) {
			...
			if (this.recipient.topic && this.recipient.topic.includes('<phone:auto-accept>')) {
				await this.accept();
				return this;
			}
			if (this.originDM) {
	...
	async accept() {
		...
		if (this.originDM || (this.origin.topic && !this.origin.topic.includes('<phone:no-notice>'))) {
		...
		if (this.originDM) {
			if (this.recipient.topic && !this.recipient.topic.includes('<phone:no-notice>')) {
	...
	async hangup(nonQuitter, validation) {
		...
		if (nonQuitter === 'time') {
			...
			const canVoicemail = this.recipient.topic && !this.recipient.topic.includes('<phone:no-voicemail>');
	...
	sendNotice(channel, dm) {
		...
		return channel.send(stripIndents`
			⚠️ **Notice:** ⚠️
			Sending illegal, NSFW, obscene, or hateful content can result in you and your server being banned from Sadge.
			To report abuse, use ${reportUsage} (please include IDs and user tags).
			${dm ? '' : 'To hide this notice, place `<phone:no-notice>` in your channel topic.'}
		`);
	}
};