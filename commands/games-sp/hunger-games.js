...
const { shuffle, removeDuplicates, verify } = require('./shared/Util');
const events = require('./hunger-games/hunger-games.json');
...
module.exports = class HungerGamesCommand extends Command {
	...
	parseEvent(event, tributes) {
		for (let i = 0; i < 6; i++) {
			const playerRegex = new RegExp(`(Player${i + 1})`, 'g');
			event = event.replace(playerRegex, `**${tributes[i]}**`);
		}
		return event;
	}
